...
import com.example.AttendanceMonitoringMobileAppTP.realm.Attendance_Students_List;
...
public class ClassDetail_Activity extends AppCompatActivity {
                        ...
                        .setListener(R.id.add_btn_popup, new View.OnClickListener() {
                            ...
                            public void onClick(View view) {
                                ...
                                if (isValid()) {
                                    ...
                                    Toast.makeText(ClassDetail_Activity.this, "Please fill all the details correctly..", Toast.LENGTH_SHORT).show();
                                }
    ...
    public void RealmInit() {
        ...
        // Initialize Realm in the current context
        Realm.init(this);

        // Get an instance of the default Realm database
        ...
        realmChangeListener = new RealmChangeListener() {
            ...
            public void onChange(Object o) {
                ...
                // Check if attendance reports exist for the given date and classID
                if (!(reports_size == 0)) {
                    // Show the layout indicating attendance has been taken
                    layout_attendance_taken.setVisibility(View.VISIBLE);
                    // Hide the submit button
                    submit_btn.setVisibility(View.GONE);
                } else {
                    // Hide the layout indicating attendance has been taken
                    layout_attendance_taken.setVisibility(View.GONE);
        ...
        // Add the realmChangeListener to listen for database changes
        realm.addChangeListener(realmChangeListener);

        // Query the database to get a RealmResults object of students with a specific class_id
        ...
        // Count the number of attendance reports for the given date and classID
        long reports_size = realm.where(Attendance_Reports.class)
                .equalTo("date_and_classID", date + room_ID)
                .count();
    ...
    public boolean isValid() {
        ...
        if (TextUtils.isEmpty(student_name.getText().toString())
                || TextUtils.isEmpty(reg_no.getText().toString())
                || TextUtils.isEmpty(mobile_no.getText().toString())) {
            return false;
        }

        if (!student_name.getText().toString().matches("[a-zA-Z'ñ Ñ ]+")) {
            Toast.makeText(this, "Please enter a valid name (letters only)", Toast.LENGTH_SHORT).show();
            return false;
        }

        if (!reg_no.getText().toString().matches("\\d{12}")) {
            Toast.makeText(this, "Please enter a valid registration number (12 digits)", Toast.LENGTH_SHORT).show();
            return false;
        }

        if (!mobile_no.getText().toString().matches("09\\d{9}")) {
            Toast.makeText(this, "Please enter a valid mobile number (start with 09)", Toast.LENGTH_SHORT).show();
            return false;
        }
